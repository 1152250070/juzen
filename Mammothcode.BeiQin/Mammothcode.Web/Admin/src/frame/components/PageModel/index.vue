...
<script>
import { commonTableDateFetch, commonAjax } from '@/utils/tools'
...
export default {
  ...
  methods: {
    ...
    */
    ...
    // 获取表格数据
    getTableData: commonTableDateFetch({
      url() {
        return this.useConfig.getUrl
      },
      data() {
        return this.getTableReqParams()
      },
      success(data) {
        const getDataCallback = this.useConfig.getDataCallback
        data = getType(getDataCallback) === 'function'
          ? getDataCallback.call(this, data)
          : data
    ...
    async onDel(row) {
      ...
      await commonAjax.call(this, true, useConfig.delUrl, {
        [delKey]: row[delKey]
      })
