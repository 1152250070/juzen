...
<template>
    ...
    <div class="wxParse" :class="className">
		<block v-for="(node, index) of nodes" :key="index" v-show="loading">
	...
	export default {
		...
		methods: {
			setHtml() {
				...
				let parseData = this.content || this.noData;
				let customHandler = {
					start: this.startHandler,
					end: this.endHandler,
					chars: this.charsHandler
				};
				let results = HtmlToJson(parseData, customHandler, this.imageProp, this);
