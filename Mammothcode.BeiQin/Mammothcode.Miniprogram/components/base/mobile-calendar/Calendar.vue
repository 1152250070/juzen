<template>
	<div class="calendar" :class="{'show': isShow,'hide': isHide}" >
		<div class="header" :class="{'show': isShow,'hide': isHide}" >
        ...
        <div class="content" v-for="(item,index) in calendar" :key="index">
            ...
            <ul class="each-month">
                <li class="each-day" v-for="(day,idx) in item.dayList" :key="idx" :class="[addClassBg(day, item.month, item.year)]" :style="{background:themeOpacityBg(day, item.month, item.year)}" @click="chooseDate(item, idx, day, item.month, item.year)">
...
<script>
    export default {
        ...
        methods: {
			...
			confirm() {
				this.$emit('confirm',this.monthSelect)
				this.isShow = false
				this.isHide = true
			},
            init() {
            ...
            createClendar() {
				...
				let _w = ''
				let dayIndex = ''
				let calendar = this.calendar[0].dayList.filter(item2 => {
					return item2
				}).map((item1,index) =>{
					_d = this.resetTime(yearTemp + '/' + monthTemp + '/' + item1)
					_w = this.weekList[new Date(_d).getDay()]
					if(item1 == this.day){
						dayIndex = index
					}
					return {
						day: item1 > 9 ? item1 : '0' + item1,
						week:item1 == this.day? '今天' : '周'+ _w
					}
				})
				let data = {
					dayList: calendar,
            ...
            chooseDate(item, index, day, month, year) {
                const _date = this.resetTime(year + '/' + month + '/' + day)
				...
				let _week = ''
				let dayIndex = ''
				let dayList = item.dayList.filter(item2 => {
					return item2
				}).map((item1,index1) =>{
					if(item1 == day) {
						dayIndex = index1
					}
					_d = this.resetTime(year + '/' + month + '/' + item1)
					_w = this.weekList[new Date(_d).getDay()]
					_week = item1 == this.day && month == this.month? '今天' : '周'+ _w
